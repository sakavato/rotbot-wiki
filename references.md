...
이 자료는 작성자가 청강한 5일 사내 교육의 키워드 메모와 **4일차 동작계획에 첨부된 로봇 제어 파이프라인 도식**을 출발점으로 삼았다. 독자와 학습 목적은 [첫 페이지](README.md#audience)에, 당시 메모에 기록된 교육 구성은 [일자별 안내](learning_path.md)에 정리했다. 원본 표현의 교정과 보완 관계는 [교육 메모의 정정과 보충](curriculum_coverage_and_gaps.md)에서 확인할 수 있다.

첨부 파이프라인은 4일차의 계획·제어 관계를 읽는 자료로 사용했다. 위키의 전체 연결도는 여러 일자의 주제와 아래 기술 자료를 종합해 재구성한 것이다. 첨부 도식의 표현 범위와 해석은 [4일차 첨부 도식 설명](curriculum_coverage_and_gaps.md#pipeline)에 정리했다.
...
| 같은 교재, [11.1 Control](https://modernrobotics.northwestern.edu/nu-gm-book-resource/11-1-control-system-overview/), [11.5 Force control](https://modernrobotics.northwestern.edu/nu-gm-book-resource/11-5-force-control/) | 폐루프와 구동, 힘 목표·관절 토크·힘 피드백 | 5 N 목표와 3 N 측정 예는 설명용이며 제어기 설정값이 아님 |
...
CI-TrajOpt의 정확한 지칭, 일부 원본 오기, 특정 제조사의 제어 방식, 개별 센서·모터의 성능은 아직 확정하지 않았다. [교육 메모의 정정과 보충](curriculum_coverage_and_gaps.md#corrections)에서 위치를 추적할 수 있다.
