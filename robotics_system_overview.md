...
먼저 위쪽의 **목표 설정·갱신**에서 무엇을 할지, 어떤 움직임을 따라갈지 정한다. 아래쪽의 **동작 중 반복되는 피드백**에서는 주어진 목표를 따라 움직이고 그 결과를 측정·보정한다. 관측한 상태는 위쪽에도 돌아가 작업 진행 판단과 재계획에 쓰인다. **제어는 유효한 목표가 주어진 동안 반복하며, 매번 상위에서 새 목표를 받을 필요는 없다.**

[연결도 크게 보기](figures/robotics_system_map.svg)

```mermaid
%%{init: {"flowchart": {"subGraphTitleMargin": {"top": 8, "bottom": 18}}}}%%
flowchart TB
    subgraph GOALS["<b>목표 설정·갱신</b>"]
        G[/"<b>작업 목표·달성 조건<br/>Goal & requirements</b><br/>물체를 손상 없이 집어 들기"/]
        T["<b>작업 순서·진행 관리<br/>Task planning / execution</b><br/>접근 → 잡기 → 들기<br/>진행·완료·실패 판단"]
        P["<b>동작·접촉 계획<br/>Motion / contact planning</b><br/>자세·경로·시간·접촉 결정<br/>관측에 따라 계획 갱신<br/>기구학·동역학·제약 반영"]
        G -.->|"작업·달성 조건"| T
        T -.->|"이번 작업 단계의 목표"| P
    end

    subgraph FEEDBACK["<b>동작 중 반복되는 피드백</b>"]
        C["<b>제어 / Control</b><br/>목표·현재 상태로 명령 계산<br/>목표 갱신 사이에도 반복 보정<br/>운동·힘 관계와 제약 반영"]
        A["<b>구동 / Actuation</b><br/>명령을 실제 힘으로 변환<br/>드라이버·모터·전달계"]
        R("<b>로봇·주변 환경<br/>Robot & environment</b><br/>몸체·물체·바닥")
        S["<b>센싱 / Sensing</b><br/>물리 현상 → 측정값<br/>센서의 신호 변환·보정"]
        E["<b>인지·상태 추정<br/>Perception / state estimation</b><br/>측정·모델로 현재 상태 계산"]
        C -->|"목표 토크 등 관절 명령"| A
        A ==>|"실제 힘·토크"| R
        E -->|"자세·속도·접촉 추정값"| C
        S -->|"측정값·시각·유효성"| E
    ...
    P -.->|"따라갈 목표 궤적·접촉 조건<br/>공급·갱신"| C
    E -->|"몸·물체·장애물 상태<br/>계획 갱신에 사용"| P
    E -->|"접촉·물체 상태 등<br/>작업 진행 판단에 사용"| T

    subgraph LEGEND["<b>표현 범례</b>"]
        direction LR
        LG[/"<b>작업 목표·달성 조건</b><br/>예: 물체를 손상 없이 집기"/] ~~~ LF["<b>로봇의 처리·동작</b><br/>계획·제어·구동·센싱·추정"] ~~~ LR("<b>로봇·주변 환경</b><br/>몸체·물체·바닥")
        LN["<b>선과 테두리의 의미</b><br/>점선: 목표 공급·갱신<br/>가는 실선: 측정·추정·명령<br/>굵은 실선: 물리적 작용·관측<br/>바깥 테두리: 설명을 위한 묶음"]
        LR ~~~ LN
    end
    E ~~~ LEGEND
...
범례는 **달성하려는 것**(흰색), **이를 위해 로봇이 하는 일**(회색), **그 과정에서 움직이고 접촉하는 몸체와 주변 환경**(연두색)을 구분한다. 예를 들어 회색의 ‘구동’은 모터가 힘을 만드는 일을, 연두색의 ‘몸체’는 그 힘을 받아 움직이는 팔·다리 등을 나타낸다. ‘센싱’은 물리 현상을 측정값으로 바꾸는 일이다. 상자 안의 **굵은 글씨는 이름**, 일반 글씨는 설명·예시다.
...
**점선 화살표**는 목표를 공급·갱신하는 관계, **가는 실선 화살표**는 측정·추정 정보나 구동 명령의 전달, **굵은 실선 화살표**는 실제 힘을 가하거나 물리 현상을 관측하는 관계다. 점선은 전달되는 값의 역할을 구분한 것으로, 실행 허가나 특정 갱신 주기를 뜻하지 않는다. 특히 `동작·접촉 계획 → 제어`의 목표 궤적, `제어 → 구동`의 목표 토크, `구동 → 로봇·주변 환경`의 실제 토크는 서로 다르다. 원하는 값을 보냈다고 그 물리적 결과가 그대로 생겼다고 판단할 수는 없다.

### 목표를 갱신하는 일과 제어를 반복하는 일

| 그림의 이름 | 결정하는 것 | 물체 집기의 예 |
|---|---|---|
| **작업 순서·진행 관리**(task planning / execution) | 할 일을 단계로 나누고, 진행 상태를 근거로 다음 단계·재시도·중단을 판단한다. | ‘접근 → 잡기 → 들기’ 중 이번에는 손을 물체에 접근시킨다. |
| **동작·접촉 계획**(motion / contact planning) | 이번 단계에 필요한 자세·경로·시간·접촉 조건을 정한다. | 물체와 장애물의 위치를 보고 손이 따라갈 목표 궤적을 만든다. |
...
절별 설명을 읽은 뒤에는 [7절의 집기 흐름](#integration)에서 이 화살표들을 하나의 작업에 적용한다.
...
손을 더 짧은 시간에 출발시키고 멈추려면 보통 더 큰 가속·감속이 필요하다. 그러면 중력을 버티는 토크 외에 운동을 바꾸는 토크도 고려해야 한다. 질량·관성·힘·토크와 움직임의 관계를 다루는 것이 **동역학**(dynamics)이다. 원하는 움직임에서 필요한 힘과 토크를 계산하는 문제를 **역동역학**(inverse dynamics)이라고 한다. [로봇 운동과 필요한 관절 토크](https://modernrobotics.northwestern.edu/nu-gm-book-resource/8-3-newton-euler-inverse-dynamics/)
...
우선 몸이 멈춰 있고, 중력과 평평한 바닥의 발 접촉 외에는 외부 힘이 없다고 가정하자. 질량중심을 바닥에 수직으로 내린 점이 지지 영역 밖에 있으면, 바닥이 발을 밀어주는 힘만으로 중력이 만드는 회전 효과를 상쇄할 수 없다. 몸이 앞으로 기울수록 발바닥 앞쪽에 힘을 더 실을 수는 있지만, 발 앞끝보다 더 앞의 바닥에서 몸을 받쳐줄 힘을 얻을 수는 없기 때문이다. 몸통이나 다리 자세를 조절하거나 발을 옮겨야 하는 이유다. 다만 이 정적인 설명만으로 걷거나 빠르게 움직일 때의 균형을 판단할 수는 없다. 가속도와 회전 운동도 함께 고려해야 한다. [질량중심·접촉력과 보행의 관계](https://underactuated.mit.edu/humanoids.html)
...
**전신 제어**(whole-body control, WBC)는 손의 작업, 몸통의 움직임, 발의 접촉 같은 여러 목표와 제약을 함께 고려해 몸 전체의 제어 입력을 정하는 접근이다. 예를 들어 손을 앞으로 보내는 동안 몸통·다리 자세와 발에 가해지는 힘을 함께 조절할 수 있다. 모든 목표를 동시에 만족할 수 없다면 제어 방식에 따라 목표의 우선순위나 움직임을 조정하며, 발 위치나 계획을 바꿔야 할 수도 있다. 팔의 동작과 몸의 지지는 이렇게 하나의 문제로 이어진다. [상태 추정·계획·전신 제어를 통합한 Atlas 연구 사례](https://dspace.mit.edu/entities/publication/6cb6cad7-9288-419c-aa5a-cc44744f2e91)
...
[전체 연결도](#system-map)의 상자 사이를 ‘선반의 물체를 집는다’는 한 작업으로 따라가 보자. 아래의 각 행은 앞 행의 결과를 쓰지만, 실행 중에는 측정과 보정이 계속 반복된다.

| 작업의 진행 | 앞에서 받은 정보로 하는 일 | 다음에 쓰이는 결과 |
|---|---|---|
| 물체와 몸의 상태를 구함 | 영상·관절각·몸의 움직임 등을 해석하고 좌표 기준을 맞춘다. | 계획이 사용할 물체·손·몸의 위치와 장애물 정보. 현재 상태는 제어에도 전달된다. |
| 손의 접근 움직임을 정함 | 물체 위치에서 잡을 방향을 정하고, 기구학으로 가능한 자세를 찾는다. 선반과의 충돌, 필요한 토크, 발의 지지 조건을 함께 검토한다. | 제어가 따라갈 손·몸의 목표 궤적과 접촉 조건. 충족할 수 없으면 자세·경로·실행 시간을 다시 정한다. |
| 계획한 움직임을 실행함 | 제어가 목표 궤적과 현재 상태를 비교해 관절 명령을 정하고, 구동부가 힘을 만든다. | 몸의 실제 움직임. 센서가 이를 다시 읽으므로 목표와 실제의 차이를 다음 입력에서 보정할 수 있다. |
| 손이 물체에 닿고 파지를 형성함 | 관측한 접촉 상태를 사용해 작업 진행과 제어 목표를 갱신한다. 위치 추종뿐 아니라 물체를 누르는 힘과 미끄럼도 고려한다. | 물체를 잡고 있는지, 들기 단계로 진행할 조건이 충족됐는지에 대한 판단. 각도·힘 중 한 값만으로 확정하지 않는다. |
| 물체를 들어 올림 | 물체가 선반에서 떨어지면 그 무게를 손과 몸이 받아야 한다. 그 영향을 동역학·균형 계산과 피드백에 반영한다. | 팔·몸통·다리의 입력과 필요하면 계획을 조정한다. 예상보다 무겁거나 미끄러지면 다음 동작의 판단도 바뀐다. |
...
<a id="deployment"></a>
로봇의 동작을 설명할 때는 지금 보는 값이 **관측한 상태인지, 앞으로의 목표인지, 구동 명령인지, 실제 물리적 결과인지**를 먼저 구분하면 된다. 그다음 이 값이 어느 좌표계·시각·단위로 표현되며 다음 기능에서 어떻게 쓰이는지 연결한다. [관측된 현상을 해석하는 예](platform_robot_walkthrough.md)
