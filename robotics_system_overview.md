...
flowchart TB
    subgraph GOALS["<b>목표 설정·갱신</b>"]
        G[/"<b>작업 목표·달성 조건<br/>Goal & requirements</b><br/>물체를 손상 없이 집어 들기<br/>관련: 7절"/]
        T["<b>작업 순서·진행 관리<br/>Task planning / execution</b><br/>접근 → 잡기 → 들기<br/>진행·완료·실패 판단<br/>관련: 7절"]
        P["<b>동작·접촉 계획<br/>Motion / contact planning</b><br/>자세·경로·시간·접촉 결정<br/>관측에 따라 계획 갱신<br/>기구학·동역학·제약 반영<br/>관련: 2·3·5·6절"]
        G -.->|"작업·달성 조건"| T
        T -.->|"이번 작업 단계의 목표"| P
    end

    subgraph FEEDBACK["<b>동작 중 반복되는 피드백 · 4절</b>"]
        C["<b>제어 / Control</b><br/>목표·현재 상태로 명령 계산<br/>목표 갱신 사이에도 반복 보정<br/>운동·힘 관계와 제약 반영<br/>관련: 4·5·6절"]
...
> **도식 연결:** [전체 연결도](#system-map)의 **동작 중 반복되는 피드백** 전체를 설명한다. 제어로 돌아오는 직접 측정값과 추정값도 이 고리의 일부다.
...
이번 예에서는 ‘물체를 손상 없이 집어 들기’라는 작업 목표를 받고, 작업 순서·진행 관리가 **접근 → 잡기 → 들기**의 단계를 정한다고 하자. 접근 위치·방향, 파지 상태, 들기 높이 등 단계별로 확인할 조건도 정해 두었다고 가정한다.

각 단계의 목표는 **작업 순서·진행 관리 → 동작·접촉 계획 → 제어**로 전달된다. 동작하는 동안 **센싱 → 인지·상태 추정**의 결과가 제어뿐 아니라 동작 계획과 작업 진행 관리에도 돌아간다. 아래 표는 이 관계를 작업 상황별로 풀어 쓴 것이며, 측정과 제어는 각 행마다 한 번씩 실행되는 것이 아니라 동작 중 계속 반복된다.

| 진행 상황 | 도식의 요소가 하는 일 | 다음에 쓰이는 결과 |
|---|---|---|
| **작업 시작·이번 단계 결정** | **작업 순서·진행 관리**가 전체 목표와 현재 상태를 바탕으로 ‘접근’ 단계를 선택한다. | **동작·접촉 계획**에 이번 단계의 목표와 확인할 조건을 전달한다. |
...
| **물체 들기·작업 완료 판단** | **계획·제어**가 물체의 하중과 전신 균형을 함께 고려하고 **구동**이 실제 힘을 만든다. **작업 순서·진행 관리**는 관측된 물체 위치·파지·몸 상태 등으로 정해 둔 완료 조건을 확인한다. [3절](#vertical) · [6절](#humanoid) | 완료 조건을 충족하면 작업 완료로 판단한다. 들어 올리기를 명령한 것과 관측 결과로 완료를 판단하는 것을 구분한다. |
