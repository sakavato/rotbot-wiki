...
아래의 **교육 주제·키워드**는 당시 메모에서 확인되는 내용을 요약한 것이다. **전체 동작과의 연결·읽을 곳**은 이해를 돕기 위해 이 자료에서 덧붙인 해설이다. 공식 강의계획서 전체를 재현한 것은 아니며, 확인할 수 있는 교육 기록을 바탕으로 안내한다.
...
| [4일차](#day-4) | 로봇 모델, 경로·궤적 계획, 추종 제어, 상호작용 제어 | 목표에서 실행할 움직임을 정하고, 실제 상태·접촉에 맞게 제어하는 관계를 살펴본다. |
...
이어 **햅틱스**(haptics)에서 사람의 촉각·운동 감각, 감각 피드백, 능동·수동 탐색과 그리퍼 등의 연구 사례를 다룬다. **동시 위치 추정 및 지도 작성**(simultaneous localization and mapping, SLAM)도 별도 주제로 기록되어 있으며, 센서별 구분, 전단(front-end)·후단(back-end), 필터링(filtering)·최적화(optimization) 접근을 포함한다.
...
전기 모터(electric motor)가 힘을 만드는 원리에서 시작해 전류(current)·토크(torque)·속도(speed)·전력(power), 감속기(speed reducer)와 기어비(gear ratio)를 다룬다. 모터의 종류로 브러시 직류 모터(brushed DC motor), 브러시리스 직류 모터(brushless DC motor, BLDC), 스테퍼 모터(stepper motor), 교류 모터(AC motor)가 기록되어 있다.

구동·제어 항목에는 피드백(feedback)·피드포워드(feedforward), 펄스 폭 변조(pulse-width modulation, PWM), H 브리지(H-bridge), 서보 모터(servo motor)가 있다. 휴머노이드·다리형 로봇·드론 등의 적용 예와 관절에 맞는 모터 선택, 특수 모터 사례로 이어진다.
...
센서(sensor)가 물리량을 신호와 숫자로 바꾸는 과정, 센서의 종류와 특성을 다룬다. 감도(sensitivity), 측정 범위(range), 분해능(resolution), 선형성(linearity), 이력 현상(hysteresis), 정밀도(precision)·정확도(accuracy) 같은 정적 특성과 응답 시간(response time)·주파수 응답(frequency response) 같은 동적 특성이 기록되어 있다.

촉각 센싱(tactile sensing)에서는 저항식(resistive)·압전식(piezoelectric)·정전용량식(capacitive)·광학식(optical) 원리와 배열·소재·구현 사례를 다룬다. 힘·토크 센서(force/torque sensor, F/T sensor), 로드셀(load cell), 스트레인 게이지(strain gauge), 센서 사양 해석도 포함된다. 활용 예에는 물체 인식, 힘 제어, 파지 안정성, 미끄럼 검출 등이 있다.
...
로봇 제어 파이프라인(robot control pipeline)을 출발점으로, 구성 공간(configuration space, C-space)·작업 공간, 기구학·동역학(dynamics), 특이점(singularity), 조작성(manipulability), 구동·센싱 구성에 따른 제어의 조건을 다룬다.
...
MeanShift·CamShift에서 시작해 칼만 필터(Kalman filter)·입자 필터(particle filter) 등의 추정 방법, 추적 평가, 딥러닝 기반 추적을 다룬다. 단일 객체 추적(single-object tracking), 다중 객체 추적(multiple-object tracking, MOT), 3차원 추적, 검출(detection)·데이터 연관(data association), SORT·DeepSORT·ByteTrack 등의 방법과 분할(segmentation) 기반 추적 사례가 기록되어 있다.
...
용어의 뜻을 찾아볼 때는 [분야별 개념](concepts.md)을 사용한다. 현재 개념 문서는 핵심 분야를 보충하는 문서다. 햅틱스의 감각 생리, 모터·촉각 센서의 세부 구현, 개별 추적·SLAM 알고리즘까지 모든 교육 키워드의 해설이 완성된 상태는 아니다. 여러 날에 등장한 용어는 공통 설명으로 연결하고, 각 일자에서는 그 용어가 어떤 주제와 함께 다뤄졌는지 설명한다.
