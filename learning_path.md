...
5일 로봇 교육의 주요 주제와 이 가이드의 관련 설명을 함께 안내한다. 수강 전에는 다룰 분야를 살펴보고, 수강 후에는 배운 용어를 전체 동작과 연결해 복습할 수 있다.

**교육 주제·키워드**는 일자별 내용을 요약한 것이며, **전체 동작과의 연결·읽을 곳**은 학습을 돕는 해설이다. 세부 키워드는 [내용 선정 기준](README.md#scope)에 따라 필요한 만큼 다룬다.

## 교육 구성 한눈에 보기

교육에서는 로봇의 몸과 운동을 표현하는 기초, 실제 힘을 만드는 구동, 접촉을 알아내는 센싱, 동작 계획과 제어, 영상으로 물체와 로봇의 상태를 알아내는 방법을 다룬다. 오버뷰에서는 이 여러 분야를 **서서 물체를 집는 휴머노이드**의 예로 연결한다.

| 일자 | 주요 교육 주제 | 전체 동작과 연결해서 볼 부분 |
|---|---|---|
...
기초 부분에서는 링크(link)·관절(joint)·말단 장치(end effector) 등 로봇의 구성 요소와 **기구학**(kinematics)을 다룬다. 주요 개념은 자유도(degrees of freedom, DOF), 관절 공간(joint space)과 작업 공간(task space), 정기구학(forward kinematics, FK)·역기구학(inverse kinematics, IK), 자코비안(Jacobian)이다.

이어 **햅틱스**(haptics)에서 사람의 촉각·운동 감각, 감각 피드백과 그리퍼 등의 연구 사례를 다룬다. **동시 위치 추정 및 지도 작성**(simultaneous localization and mapping, SLAM)도 별도 주제로 다루며, 센서별 구분, 전단(front-end)·후단(back-end), 필터링(filtering)·최적화(optimization) 접근을 포함한다.
...
전기 모터(electric motor)가 힘을 만드는 원리에서 시작해 전류(current)·토크(torque)·속도(speed)·전력(power), 감속기(speed reducer)와 기어비(gear ratio)를 다룬다. 모터 종류별 특성과 선택 기준도 함께 살펴본다.
...
센서(sensor)가 물리량을 신호와 숫자로 바꾸는 과정, 센서의 종류와 특성을 다룬다. 측정 범위(range)·분해능(resolution)·정밀도(precision)·정확도(accuracy) 같은 정적 특성과 응답 시간(response time) 같은 동적 특성을 살펴본다.
...
5일차는 **시각 객체 추적**(visual object tracking)과 **시각 SLAM**(visual SLAM)을 나누어 살펴본다.
...
카메라·라이다(LiDAR)·관성 측정 장치(inertial measurement unit, IMU) 등 센서, 전단의 특징 추출(feature extraction)·대응·운동 추정, 후단의 필터링·최적화·지도 관리를 다룬다. 루프 폐합(loop closure), 시각·관성 주행 추정(visual-inertial odometry, VIO), 지도 표현, 운동으로부터의 구조 복원(structure from motion, SfM)과 관련 도구·연구 사례도 포함한다.
